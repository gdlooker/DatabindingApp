...
import com.example.dell.databindingapp.cache.DiskLruCache;
import com.example.dell.databindingapp.cache.LruCache;
import com.example.dell.databindingapp.call.MergeCallBack;
import com.example.dell.databindingapp.config.ImageUrlConfig;
import com.example.dell.databindingapp.enity.Result;
...
public class JImageLoader {
    ...
    private static final int IMG_URL = R.id.img_url_tag;//只是一个点位id,没有其它作用。
    ...
    public void displayImages(final List<String> urls, final ImageView imageView, final MergeCallBack mergeCallBack) {
        try {
            ...
            Log.e(TAG, "displayImages error>>" + e.getMessage());
        }
    }
    ...
    public void displayImages(final List<String> urls, final ImageView imageView, final MergeCallBack mergeCallBack, final int dstWidth, final int dstHeight) {
        ...
        Bitmap bitmap = loadFromMemory(url);
        if (bitmap != null) {
            Log.i(Tag, "displayImages this is from Memory");
            imageView.setImageBitmap(bitmap);
            return;
        }
        ...
        try {
            ...
            if (bitmap != null) {
                Log.i(Tag, "displayImages this is from Disk");
                imageView.setImageBitmap(bitmap);
                return;
            }
        } catch (Exception e) {
...
//        设置一张默认图
        if(defaultId != 0) {
            bitmap = BitmapFactory.decodeResource(mContext.getResources(), defaultId);
            imageView.setImageBitmap(bitmap);
        }

        Log.i(Tag, "displayImages this is from default");
                ...
                if (bitmaps != null && bitmaps.size() > 0) {
                    ...
                    if (mergeCallBack != null) {
                        ...
                        if (urls.size() == bitmaps.size()) {
                            ...
                            Log.i(Tag, "size change. so can not save");
                        }


                        Log.i(Tag, "displayImages this is from Merge");
     ...
     * @return
     */
    private synchronized Bitmap loadBitMap(String url, int dstWidth, int dstHeight) {
        Bitmap bitmap = loadFromMemory(url);
        if (bitmap != null) {
            Log.i(Tag, "this is from Memory");
            return bitmap;
        }
        ...
        try {
            bitmap = loadFromDiskCache(url, dstWidth, dstHeight);
            if (bitmap != null) {
                Log.i(Tag, "this is from Disk");
                return bitmap;
            }

            bitmap = loadFromNet( ImageUrlConfig.INSTANCE.toHeader123(url) , dstWidth, dstHeight);
            Log.i(Tag, "this is from Net");
            if (bitmap == null) {
                Log.i(Tag, "bitmap null network error");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private synchronized ArrayList<Bitmap> loadBitMaps(List<String> urls, int dstWidth, int dstHeight) {
        ArrayList<Bitmap> bitmaps = new ArrayList<>();
        List<String> tempUrls = new ArrayList<>(urls);
    ...
    private Bitmap loadFromMemory(String url) {
        Log.i(Tag, "this is from memory:key=" + getKeyFromUrl(url));
